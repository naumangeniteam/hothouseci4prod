...
class Adminmanagepublishcomedyandspokenword extends BaseController {
...
$this->lang->setLocale('admin');
	...
	{	
		...
		$data['error'] 						= 	'';
		$data['activeMenu'] 				= 	'adminmanagepublishcomedyandspokenword';
		$data['activeSubMenu'] 				= 	'adminmanagepublishcomedyandspokenword';
		...
		$baseUrl 							= 	base_url().'hhjsitemgmt/adminmanagepublishcomedyandspokenword/index';
       ...
       if ($this->request->getUri()->getSegment($uriSegment)) :
		...
		$this->layouts->admin_view('publishcomedyandspokenword/index',array(),$data);
	}	// END OF FUNCTION
	...
	{		
		...
		if($this->request->getPost('SaveChanges')):
			$error					=	'NO';
            $this->form_validation->set_rules('event_title', 'event Title', 'trim|required|max_length[256]');
			...
			if($this->form_validation->run() && $error == 'NO'): 
				...
				if($this->request->getPost('CurrentDataID') ==''):
					...
					$this->common_model->editData('comedyandspokenword_tbl',$param,'event_id',(int)$Id);
