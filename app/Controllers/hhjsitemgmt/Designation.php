...
$this->lang->setLocale('admin');
	...
	{	
		...
		if($this->request->getGet('searchValue')):
			...
			$whereCon['like']			 	= 	"(ftable.designation_name LIKE '%".$sValue."%')";
		...
		$whereCon['where']		 			= 	"";		
		$shortField 						= 	"ftable.designation_name ASC";
		
		$baseUrl 							= 	getCurrentControllerPath('index');
		$this->session->set('designationAdminILCADMData',currentFullUrl());
		$qStringdata						=	explode('?',currentFullUrl());
		$suffix								= 	$qStringdata[1]?'?'.$qStringdata[1]:'';
		$tblName 							= 	'admin_designation as ftable';
		$con 								= 	'';
	...
	{		
		...
		if($editId):
			...
			$data['EDITDATA']		=	$this->common_model->getDataByParticularField('admin_designation','designation_id',(int)$editId);
		else:
		...
		if($this->request->getPost('SaveChanges')):
			$error					=	'NO';
			$this->form_validation->set_rules('designation_name', 'Name', 'trim|required|is_unique[admin_designation.designation_name.designation_id]');
			
			if($this->form_validation->run() && $error == 'NO'):   
			
				$param['designation_name']		= 	addslashes($this->request->getPost('designation_name'));
				$param['designation_slug']		= 	url_title(strtolower($this->request->getPost('designation_name')));
				
				if($this->request->getPost('CurrentDataID') ==''):
					$param['designation_used']	=	'N';
					$param['creation_ip']		=	currentIp();
					$param['creation_date']		=	(int)$this->timezone->utc_time();//currentDateTime();
					$param['created_by']		=	(int)$this->session->get('CMPOP_ADMIN_ID');
					$param['status']			=	'A';
					$alastInsertId				=	$this->common_model->addData('admin_designation',$param);
					$this->session->setFlashdata('alert_success',lang('statictext_lang.addsuccess'));
				else:
					$departmentId				=	$this->request->getPost('CurrentDataID');
					$param['update_ip']			=	currentIp();
					$param['update_date']		=	(int)$this->timezone->utc_time();//currentDateTime();
					$param['updated_by']		=	(int)$this->session->get('CMPOP_ADMIN_ID');
					$this->common_model->editData('admin_designation',$param,'designation_id',(int)$departmentId);
					$this->session->setFlashdata('alert_success',lang('statictext_lang.updatesuccess'));
				endif;
				
				redirect(correctLink('designationAdminILCADMData',getCurrentControllerPath('index')));
			endif;
		endif;
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->editData('admin_designation',$param,'designation_id',(int)$changeStatusId);
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->deleteData('admin_designation','designation_id',(int)$deleteId);
