...
class Adminmanageaboutus extends BaseController {
	...
	{	
		...
		$data['error'] 						= 	'';
		$data['activeMenu'] 				= 	'adminmanageaboutus';
		$data['activeSubMenu'] 				= 	'adminmanageaboutus';
		...
		$tblName 							= 	'about_us_tbl as ftable';
		$con 								= 	'';
		//echo $tblName; die();
		...
		$this->layouts->set_title('Manage About Us');
		$this->layouts->admin_view('aboutus/index',array(),$data);
	}	// END OF FUNCTION
	...
	{		
		...
		$data['activeMenu'] 				= 	'adminmanageaboutus';
		$data['activeSubMenu'] 				= 	'adminmanageaboutus';
		$data['EDITDATA']		=	$this->common_model->getDataByParticularField('about_us_tbl','id',(int)$editId);
		...
		if($this->request->getPost('SaveChanges')):
			if($this->request->getPost('image') == ''){
				session()->setFlashdata('alert_error',lang('statictext_lang.uploadimgerror'));
			}
			...
			$messages = [
				'page_title' => [
					...
					'max_length' => 'Venue Title must be within 256 characters.'
				],
				'content' => ['required' => 'Content is required.']
			];
	
			// Image validation (only required if no existing image is present)
			...
			if ($file && $file->isValid() && !$file->hasMoved()) {
				...
				$file->move('assets/front/img/aboutus', $newName);
				$param['image'] = $newName;
			}
	
	
				$param['page_title']				= 	$this->request->getPost('page_title');
				...
				if($this->request->getPost('CurrentDataID') ==''):
					...
					$param['is_active']			=	'1'; 
					$alastInsertId				=	$this->common_model->addData('about_us_tbl',$param);
					...
					$this->common_model->editData('about_us_tbl',$param,'id',(int)$Id);
		...
		$this->layouts->set_title('Manage About Us');
		$this->layouts->admin_view('aboutus/addeditdata',array(),$data);
	}	// END OF FUNCTION	
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->editData('about_us_tbl',$param,'id',(int)$changeStatusId);
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->deleteData('about_us_tbl','id',(int)$deleteId);
