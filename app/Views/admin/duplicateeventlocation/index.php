      ...
      <div class="page-block">
        <div class="row align-items-center">
          <div class="col-md-12">
            ...
            <ul class="breadcrumb">
              <li class="breadcrumb-item"><a href="javascript:void(0);">Duplicate Venue List</a></li>
            </ul>
          </div>
        </div>
      </div>
                    ...
                    <div class="col-sm-12 col-md-4">
                      <div class="row">
                        <div class="form-group ">
                          <select name="county" id="county" style="padding: 2px;" class="form-control">
                        ...
                        <!-- <a href="<?php echo base_url('hhjsitemgmt/adminmanageeventlocation/export_excel'); ?>" class="btn btn-sm btn-primary pull-right" style="margin-left: 10px; margin-top: 5px;height: 40px;">Export</a> -->
                      </div>
                    </div>
                      ...
                      <select name="bulk_option" style="margin:4px;padding:2px;" onchange="moveToVenue()">
                        ...
                        if (!empty($venues)) :
                          foreach ($venues as $venues) : ?>
                            <option value="<?= $venues->id ?>"><?= $venues->venue_title; ?></option>
                        <?php endforeach;
                        endif; ?>
                      </select>
                    </div> -->
                          ...
                          <tbody>
                            <?php if ($ALLDATA <> "") : $i = $first;
                              ...
                              foreach ($ALLDATA as $ALLDATAINFO) :
                                if ($ALLDATAINFO['added_by'] != 'user' || $ALLDATAINFO['is_active'] != '0') {
                                  //$getVenuName = $this->db->select('venue_title', 'position')->from('venue_tbl')->where('id', $ALLDATAINFO['venue_id'])->get()->row();
                               ...
                               // $getEventLocationName = $this->db->select('name', 'is_active')->from('event_location_type')->where('id', $ALLDATAINFO['event_location_type_id'])->get()->row();
                                $getEventLocationName = $db->table('event_location_type')
                            ...
                            ?>
                                <tr role="row" style="<?php echo $ALLDATAINFO['is_front'] == '1' ? 'background-color:  #ffe6e6;' : ''; ?>" class="<?php echo $rowClass; ?>">
                                  ...
                                  <td><?= stripslashes(strip_tags($ALLDATAINFO['id'])) ?></td>
                                  ...
                                  <td>
                                    <?php
                                    if (empty($ALLDATAINFO['location_source'])) {
                                      echo "Admin";
                                    } elseif ($ALLDATAINFO['location_source'] === 'front_user') {
                                      echo "User";
                                    } elseif ($ALLDATAINFO['location_source'] === 'ticketmaster') {
                                      echo "Ticketmaster";
                                    }
                                    ?>
                                  </td>
...
<!-- <script>
  function moveToVenue() {
    ...
    fetch('<?php echo base_url('hhjsitemgmt/adminmanageeventlocation/moveToVenue'); ?>', {
        method: 'POST',
        headers: {
