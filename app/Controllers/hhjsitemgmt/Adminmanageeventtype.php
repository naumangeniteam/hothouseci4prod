...
class Adminmanageeventtype extends BaseController {
	...
	{	
		...
		$data['error'] 						= 	'';
		$data['activeMenu'] 				= 	'adminmanageeventlocationtype';
		$data['activeSubMenu'] 				= 	'adminmanageeventlocationtype';
		...
		$tblName 							= 	'event_location_type as ftable';
		$con 								= 	'';
		//echo $tblName; die();
		...
		$this->layouts->set_title('Manage Location Event Types');
		$this->layouts->admin_view('event_location_type/index',array(),$data);
	}	// END OF FUNCTION
	...
	{		
		...
		if($editId):
			...
			$data['EDITDATA']		=	$this->common_model->getDataByParticularField('event_location_type','id',(int)$editId);
		else:
		...
		if($this->request->getPost('SaveChanges')):
            ...
            // $this->form_validation->set_rules('buy_now_link', 'Buy Now Link', 'trim|required');
            // $this->form_validation->set_rules('website_link', 'Website Link', 'trim|required');
            // $this->form_validation->set_rules('artist_bio', 'Artist Bio', 'trim|required');
			...
			if (!$this->validate($validationRules)) {
				...
				if($this->request->getPost('CurrentDataID') ==''):
					...
					$param['is_active']			=	'1'; 
					$alastInsertId				=	$this->common_model->addData('event_location_type',$param);
					...
					$this->common_model->editData('event_location_type',$param,'id',(int)$Id);
        ...
        $this->layouts->set_title('Manage Event Location Types');
		$this->layouts->admin_view('event_location_type/addeditdata',array(),$data);
	}	// END OF FUNCTION	
	...
	{  
		...
		$this->common_model->editData('event_location_type',$param,'id',(int)$changeStatusId);
	...
	{  
		...
		$this->common_model->deleteData('event_location_type','id',(int)$deleteId);
