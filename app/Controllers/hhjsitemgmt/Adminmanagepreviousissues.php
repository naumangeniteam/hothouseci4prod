...
class Adminmanagepreviousissues extends BaseController {
	...
	{	
		...
		$data['error'] 						= 	'';
		$data['activeMenu'] 				= 	'adminmanagepreviousissues';
		$data['activeSubMenu'] 				= 	'adminmanagepreviousissues';
		
		if($this->request->getGet('searchValue')):
			$sValue							=	$this->request->getGet('searchValue');
			$whereCon['like'] = [
				'ftable.venue_title' => $sValue
			  ];
		...
		$tblName 							= 	'previous_issues_tbl as ftable';
		$con 								= 	'';
		//echo $tblName; die();
		...
		$this->layouts->set_title('Manage Previous issues');
		$this->layouts->admin_view('previousissues/index',array(),$data);
	}	// END OF FUNCTION
	...
	{		
		...
		if($editId):
			...
			$data['EDITDATA']		=	$this->common_model->getDataByParticularField('previous_issues_tbl','id',(int)$editId);
		else:
		...
		if($this->request->getPost('SaveChanges')):
            ...
            $validationRules = [
				...
				if($this->request->getPost('CurrentDataID') ==''):
					...
					$param['is_active']			=	'1'; 
					$alastInsertId				=	$this->common_model->addData('previous_issues_tbl',$param);
					...
					$this->common_model->editData('previous_issues_tbl',$param,'id',(int)$Id);
		...
		$this->layouts->set_title('Manage Previous Issues');
		$this->layouts->admin_view('previousissues/addeditdata',array(),$data);
	}	// END OF FUNCTION	
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->editData('previous_issues_tbl',$param,'id',(int)$changeStatusId);
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->deleteData('previous_issues_tbl','id',(int)$deleteId);
