...
class Adminmanagefooter extends BaseController {
	...
	{	
		...
		$data['error'] 						= 	'';
		$data['activeMenu'] 				= 	'adminmanagefooter';
		$data['activeSubMenu'] 				= 	'adminmanagefooter';
		
		if($this->request->getGet('searchValue')):
			$sValue							=	$this->request->getGet('searchValue');

			$whereCon['like'] = ['ftable.page_title' => $sValue];
		...
		$this->layouts->admin_view('footer/index',array(),$data);
	}	// END OF FUNCTION
	...
	{		
		...
		if($editId):
			...
			$data['EDITDATA']		=	$this->common_model->getDataByParticularField('footer_tbl','id',(int)$editId);
		else:
		...
		if($this->request->getPost('SaveChanges')):
			...
			if (!$this->validate($validationRules)) {
				return redirect()->back()->withInput()
					->with('validation', $validation); // ✅ Store validation errors
			}
				...
				if($this->request->getPost('CurrentDataID') ==''):
					...
					$param['is_active']			=	'1'; 
					$alastInsertId				=	$this->common_model->addData('footer_tbl',$param);
					...
					$this->common_model->editData('footer_tbl',$param,'id',(int)$Id);
		...
		$this->layouts->set_title('Manage Footer');
		$this->layouts->admin_view('footer/addeditdata',array(),$data);
	}	// END OF FUNCTION	
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->editData('footer_tbl',$param,'id',(int)$changeStatusId);
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->deleteData('footer_tbl','id',(int)$deleteId);
