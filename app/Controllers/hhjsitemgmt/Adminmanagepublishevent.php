...
class Adminmanagepublishevent extends BaseController {
...
$this->lang->setLocale('admin');
	...
	{	
		...
		$data['error'] 						= 	'';
		$data['activeMenu'] 				= 	'adminmanagepublishevent';
		$data['activeSubMenu'] 				= 	'adminmanagepublishevent';
		...
		$baseUrl 							= 	base_url().'hhjsitemgmt/adminmanagepublishevent/index';
		...
		$page = ($uri->getTotalSegments() >= $uriSegment) ? $uri->getSegment($uriSegment, 1) : 0;
    ...
    //    endif;
		...
		$this->layouts->admin_view('publishevent/index',array(),$data);
	}	// END OF FUNCTION
		...
		if($this->request->getPost('SaveChanges')):
			...
			if($this->form_validation->run() && $error == 'NO'): 
				...
				if($this->request->getPost('CurrentDataID') ==''):
					...
					$this->common_model->editData('event_tbl',$param,'event_id',(int)$Id);
