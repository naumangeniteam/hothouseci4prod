...
class Adminmanageadvertisement extends BaseController {
	...
	{	
		...
		$data['error'] 						= 	'';
		$data['activeMenu'] 				= 	'adminmanageadvertisement';
		$data['activeSubMenu'] 				= 	'adminmanageadvertisement';
		...
		$tblName 							= 	'advertisement_tbl as ftable';
		$con 								= 	'';
		//echo $tblName; die();
		...
		$this->layouts->set_title('Manage Advertisement Section');
		$this->layouts->admin_view('advertisement/index',array(),$data);
	}	// END OF FUNCTION
	...
	{		
		...
		$data['activeMenu'] 				= 	'adminmanageadvertisement';
		$data['activeSubMenu'] 				= 	'adminmanageadvertisement';
		$data['EDITDATA']		=	$this->common_model->getDataByParticularField('advertisement_tbl','id',(int)$editId);
		if($editId):
		...
		if($this->request->getPost('SaveChanges')):
			...
			$rules = [
				'title1'   => 'trim|required|max_length[256]',
				'title2'   => 'trim|required|max_length[256]',
			...
			$messages = [
				'title1' => [
					...
					'max_length' => 'Title1 should not exceed 256 characters.'
				],
				'title2' => [
					'required'   => 'Title2 is required.',
					'max_length' => 'Title2 should not exceed 256 characters.'
				],
				'content1' => [
					'required' => 'Content1 is required.'
				],
				'content2' => [
					'required' => 'Content2 is required.'
				]
			];
			...
			// if($this->form_validation->run() && $error == 'NO'): 
				$param['title1']				= 	$this->request->getPost('title1');
                $param['title2']				= 	$this->request->getPost('title2');
				...
				if($this->request->getPost('CurrentDataID') ==''):
					...
					$param['is_active']			=	'1'; 
					$alastInsertId				=	$this->common_model->addData('advertisement_tbl',$param);
					...
					$this->common_model->editData('advertisement_tbl',$param,'id',(int)$Id);
        ...
        $this->layouts->set_title('Manage Advertisement');
		$this->layouts->admin_view('advertisement/addeditdata',array(),$data);
	}	// END OF FUNCTION	
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->editData('advertisement_tbl',$param,'id',(int)$changeStatusId);
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->deleteData('advertisement_tbl','id',(int)$deleteId);
