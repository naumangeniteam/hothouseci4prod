  ...
  <div class="pcoded-content">
      ...
      <div class="page-block">
        <div class="row align-items-center">
          <div class="col-md-12">
            ...
            <ul class="breadcrumb">
              <li class="breadcrumb-item"><a href="javascript:void(0);"> Imported Events List</a></li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div class="col-sm-12">
        <div class="card">
          <div class="card-header">
            ...
            <!-- <a href="<?php echo base_url('hhjsitemgmt/eventmanagement/addeditdata'); ?>" class="btn btn-sm btn-primary pull-right">Add Content</a> -->
          </div>
          <div class="row">
                ...
                <div class="widget-body bordered-left bordered-warning">
                  <form class="form-inline" method="get" id="validate" novalidate="novalidate">
                    ...
                    <div class="form-group">
                      ...
                      <a href="<?= base_url('hhjsitemgmt/adminmanageapiimport') ?>" class="btn btn-link">View All </a>
                    </div>
                  </form>
                  ...
                  <div class="row">
                    ...
                    <div class="col-sm-12 col-md-10 col-xs-12">
                      <div class="row" style="margin:3px;">
                        <select name="bulk_option" style="padding:2px;" onchange="callMutlipleChangeStatus()">
                          <option value="">Bulk Option Api Import Events</option>
                          <option value="saveLoc">Save Data to Event</option>
                        ...
                        <table id="simpletable" class="table table-striped table-bordered nowrap dataTable custom-data-table" role="grid" aria-describedby="simpletable_info">
                          <thead class="text-nowrap">
                            <tr role="row">
                              ...
                              <th>Event Title</th>
                              <th>Venue</th>
                              <th>Location<br>Address</th>
                              <th>Db Matching Loc</th>
                              <th>DB Venue</th>
                              <th>Accept <br>Db Loc</th>
                              <th>Start / <br>
                              End</th>
                              <!-- <th>Creation Date</th>
                              <th>Updated Date</th> -->
        ...
        $getVenuName = $query->venue_title ?? 'N/A';
                                  $dbVenuName = $this->db->select('venue_title')->from('venue_tbl')->where('id', $ALLDATAINFO['db_venue'])->get()->row();
                            ?>
                                  <tr role="row" class="<?php echo $rowClass; ?>">
                                    <td style="width:50px;"><input type="checkbox" name="checkbox_mutli" class="row-checkbox" value="<?= $ALLDATAINFO['event_id'] ?>"></td>
                                    <td style="width:50px;"><?= $i++ ?></td>
                                    <td style="width:100px;"><?= stripslashes(strip_tags($ALLDATAINFO['event_title']))?></td>
                                    <td style="width:70px;"><?= stripslashes(strip_tags($getVenuName->venue_title)) ?></td>
                                    <td style="width:70px;"><?= stripslashes(strip_tags($ALLDATAINFO['location_name'])) .',<br><br> '.stripslashes(strip_tags($ALLDATAINFO['location_address'])).',<br><br> '.stripslashes(strip_tags($ALLDATAINFO['latitude'])).'<br>'.stripslashes(strip_tags($ALLDATAINFO['longitude']))?></td>
                                    <td style="width:70px;">   <?php 
    if (!empty($ALLDATAINFO['match_loc'])) { 
        echo strip_tags($ALLDATAINFO['match_loc']) . ',<br><br> ' . 
             stripslashes(strip_tags($ALLDATAINFO['match_add'])) . ',<br><br> ' . 
             stripslashes(strip_tags($ALLDATAINFO['match_loc_lat'])) . ',<br> ' . 
             stripslashes(strip_tags($ALLDATAINFO['match_loc_long']));
    }
    ?></td>
                                    <td style="width:70px;"><?= strip_tags($dbVenuName->venue_title) ?></td>
                                    <td style="width:50px;"> <?php 
    if (!empty($ALLDATAINFO['match_loc'])) { 
        echo '<input type="checkbox" name="checkbox_loc" class="row-loc-checkbox" checked value="' . $ALLDATAINFO['loc_id'] . '">';
    } 
    else
    {
    echo $sel_venue;
    ...
    ?></td>
                                    ...
                                    <td style="width:120px;">
                                      <?= stripslashes(strip_tags($ALLDATAINFO['start_date'])) .' '. strip_tags($ALLDATAINFO['event_start_time']). ' <br> /' . stripslashes(strip_tags($ALLDATAINFO['end_date'])).' '. strip_tags($ALLDATAINFO['event_end_time']) ?>
                                    </td>
                                    ...
                                    <td style="width:100px;">
                                      <div class="btn-group">
                                        ...
                                        <li><a href="<?php echo base_url('hhjsitemgmt/adminmanageapiimport/addeditdata/' . $ALLDATAINFO['event_id']) ?>"><i class="fas fa-edit"></i> Edit Details</a></li>
                                          <?php if ($ALLDATAINFO['is_active'] == '1') : ?>
                                            <li><a href="<?php echo base_url('hhjsitemgmt/adminmanageapiimport/changestatus/' . $ALLDATAINFO['event_id'] . '/0') ?>" onClick="return confirm('Want to Inactive!');"><i class="fas fa-thumbs-down"></i> Inactive</a></li>
                                          <?php elseif ($ALLDATAINFO['is_active'] == '0') : ?>
                                            <li><a href="<?php echo base_url('hhjsitemgmt/adminmanageapiimport/changestatus/' . $ALLDATAINFO['event_id'] . '/1') ?>" onClick="return confirm('Want to Active!');"><i class="fas fa-thumbs-up"></i> Active</a></li>
                                          <?php endif; ?>
                                          <?php if ($ALLDATAINFO['is_imported'] == '1') : ?>
                                            <li><a href="<?php echo base_url('hhjsitemgmt/adminmanageapiimport/updateimport/' . $ALLDATAINFO['event_id'] . '/0') ?>" onClick="return confirm('Want to Not Imported!');"><i class="fas fa-thumbs-down"></i> Not Imported</a></li>
                                          <?php elseif ($ALLDATAINFO['is_imported'] == '0') : ?>
                                            <li><a href="<?php echo base_url('hhjsitemgmt/adminmanageapiimport/updateimport/' . $ALLDATAINFO['event_id'] . '/1') ?>" onClick="return confirm('Want to Imported!');"><i class="fas fa-thumbs-up"></i> Imported</a></li>
                                          <?php endif; ?>
                                          <li><a href="<?php echo base_url('hhjsitemgmt/adminmanageapiimport/deletedata/' . $ALLDATAINFO['event_id']) ?>" onClick="return confirm('Want to delete Permanent!');"><i class="fas fa-trash"></i>Permanent Delete</a></li>
                                        </ul>
                                      </div>
                                    </td>
                                  </tr>
...
<script>
  document.getElementById('checkAll').addEventListener('change', function() {
    ...
    var checkboxes = document.querySelectorAll('tbody input[name="checkbox_mutli"]');
...
<script>
  function callMutlipleChangeStatus() {
    ...
    document.querySelectorAll('input[name="checkbox_mutli"]:checked').forEach((checkbox) => {
            let eventId = checkbox.value;
            let row = checkbox.closest("tr");
            let locCheckbox = row.querySelector('input[name="checkbox_loc"]');
            let venueSelect = row.querySelector("select");
            let matchSel ="No";
            let matchId ="";
            if (locCheckbox) {
                matchSel = locCheckbox.checked ? "DB Checked" : "No";
                if(matchSel !="No")
                {
                matchId= locCheckbox.value;
                }
            } else if (venueSelect.value) {
              matchSel = "Venue Selected" || "No";
              matchId=  venueSelect.value;
            }

            selectedData.push({ event_id: eventId, match_sel: matchSel,match_id: matchId});
        });
   ...
   if (statusType === "permanentdelete") {
    ...
    fetch('<?php echo base_url('hhjsitemgmt/adminmanageapidata/multipleChangeStatus'); ?>', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
        },
        body: 'selectedData=' + JSON.stringify(selectedData) + '&statusType=' + statusType
      })
      .then(response => {
