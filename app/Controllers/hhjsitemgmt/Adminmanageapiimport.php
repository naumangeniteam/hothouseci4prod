...
class Adminmanageapiimport extends BaseController
{
	...
	public function index()
	{
		...
		$data['error'] 						= 	'';
		$data['activeMenu'] 				= 	'adminmanageapiimport';
		$data['activeSubMenu'] 				= 	'adminmanageapiimport';
		...
		$baseUrl = base_url() . 'hhjsitemgmt/adminmanageapiimport/index';

			// Get the query string (e.g., "?page=2")
		...
		$totalRows = $this->common_model->getData_api_import_event('count', $tblName, $whereCon, $shortField, '0', '0');
		...
		$data['ALLDATA'] 					= 	$this->common_model->getData_api_import_event('multiple', $tblName, $whereCon, $shortField, $perPage, $offset);
		...
		$this->layouts->admin_view('event/api_import', array(), $data);
	}	// END OF FUNCTION
	...
	public function addeditdata($editId = '')
	{
		...
		if ($this->request->getPost('SaveChanges')) {
				...
				// Apply validation
				if (!$this->validate($validationRules) || $error !== 'NO') {
					return redirect()->back()->withInput()->with('validation', $this->validator);
				}
		
				// Handle File Upload
				$eventImage = $this->request->getFile('event_source_image');
				if ($eventImage && $eventImage->isValid() && !$eventImage->hasMoved()) {
					$newName = $eventImage->getRandomName(); // Generate a unique file name
					$eventImage->move('assets/front/img/eventimage', $newName);
		...
		$this->layouts->set_title('Manage Api Import');
		$this->layouts->admin_view('event/addeditdata_api_import', array(), $data);
	}	// END OF FUNCTION	
	...
	{
		$importedId = $this->uri->getSegment(4); 
		$statusType = $this->uri->getSegment(5);
	...
	function mutlipleChangeStatus()
	{
		...
		if ($statusType !== "permanentdelete") {
			...
			foreach ($changeStatusIds as $changeStatusId) {
				...
				// else if ($statusType == 'importEvent') {
				// 	$param['is_imported'] = "1";
				// }
	...
	{
		$date_value = $this->uri->getSegment(4);
